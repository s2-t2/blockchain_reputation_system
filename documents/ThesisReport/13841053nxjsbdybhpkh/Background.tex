...
%In a \ac{DHT}~\cite{balakrishnan2003looking,tanenbaum2007distributed} based
%system, the data items are assigned a random key from a large identifier space,
%such as 128-bit or 160-bit identifier. Similarly, nodes in the system are also
%assigned a random number from the same identifier space. The \ac{DHT} then
%should provide a lookup service in a distributed system, i.e., Given a data
%item $X$ stored at some dynamic set of nodes in the system, find it. The crux
%of every DHT-based system is then to implement an efficient and deterministic
%scheme that uniquely maps the key of a data item to the identifier of a node
%based on some distance metric~\cite{tanenbaum2007distributed}. \par 
...
%A malicious node attempting to
...
several branches of the blockchain. In which case, the nodes build upon the
