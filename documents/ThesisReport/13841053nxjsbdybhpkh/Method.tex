...
information. The model addresses different concerns such as roles and
requirements of participants as endorser and endorsee and several possible
behaviors of an interaction system. A system of smart contact is set up that
specifies the interaction between entities, aggregates interaction information
and assigns a trust score to each participating entities. The storage of data
both on and off-chain along with the governance model is discussed. 
...
The centralized nature of current online systems fails to provide the guarantee
of reliable and immutable data. As such, the reputation information is
vulnerable to external attacks as well as internal modifications. Additionally,
the reputation systems do not take into account the anonymity/pseudonymity of
participants, which is an essential attribute for getting honest feedback. 
This masters' thesis proposes the use of blockchain technology for storage and
governance of reputation data to ensure reliable and verifiable information. By
modeling trust among entities in a pseudonymous manner, the proposed system
also considers the user's anonymity requirements. 
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{Images/ContextLayer.eps}
...
\begin{wrapfigure}{l}{0.6\textwidth}
	...
	\end{center}
\end{wrapfigure}
based on the assumption that can be used by endorsement system.  Thus,
...
with every endorsement connection made along the way. As can be seen in
...
\acrshort{CP}$_A$: \ac{CP} of a participant 'A' that updates with each
