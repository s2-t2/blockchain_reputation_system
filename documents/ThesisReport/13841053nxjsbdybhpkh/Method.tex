...
behaviors of an interaction system. A system of smart contract is set up that
...
The centralized nature of current online systems fail to provide the guarantee
...
This master's thesis project proposes the use of blockchain technology for storage and
...
also considers the users' anonymity requirements. 
...
\textit{$ratio_A$} is always assumed to be less than or equal to 1 and is given by: 
\begin{equation}
	ratio_A = \frac{min(nEG_A,nER_A)}{max(nEG_A,nER_A)} 
\end{equation}

\acrshort{CP}$_A$: \ac{CP} of a participant \textit{'A'} that updates with each
